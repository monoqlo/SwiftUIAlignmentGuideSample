...
fileprivate struct Challenge4Answer: View {
    ...
    var body: some View {
        ZStack(alignment: .topMinorTextRankTrailing) {
                Text(rank.majorText)
            ...
            if let minorText = rank.minorText {
                Text(minorText)
                    ...
                    .alignmentGuide(.minorTextRankTrailing) { context in
...
extension HorizontalAlignment {
    fileprivate enum MinorTextRankAlignment: AlignmentID {
        static func defaultValue(in context: ViewDimensions) -> CGFloat {
            context[HorizontalAlignment.trailing]
        }
    }
    
    fileprivate static let minorTextRankTrailing = HorizontalAlignment(
        MinorTextRankAlignment.self)
}
...
extension Alignment {
    ...
    fileprivate static let topMinorTextRankTrailing = Alignment(
        horizontal: HorizontalAlignment(HorizontalAlignment.MinorTextRankAlignment.self),
        vertical: .top
    )
}
